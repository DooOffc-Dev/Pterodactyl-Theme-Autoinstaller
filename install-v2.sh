...
check_token() {
  echo -e "                                                       "
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "${BLUE}[+]                   LICENSY DooOfficiall                     [+]${NC}"
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "                                                       "
  echo -e "${YELLOW}MASUKAN AKSES TOKEN :${NC}"
  read -r USER_TOKEN

  if [ "$USER_TOKEN" = "aloffc" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di DooOfficiall${NC}"
    echo -e "${YELLOW}TELEGRAM : @DooOfficiall${NC}"
    echo -e "${YELLOW}TELEGRAM2 : @DooOffc${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 15K ${NC}"
    echo -e "${YELLOW}© DooOfficiall${NC}"
    exit 1
  fi
  clear
}
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer DooOfficiall   ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : DooOfficiall      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : DooOfficiall  ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by DooOfficiall  ${NC}"
